...
| In the Pages frontend, `/open_api/settings` shows `CORS error` in DevTools or returns `429 error code 1027` | Do not point the frontend directly at `https://api.xlyyds.cloud`. Use Pages Functions to proxy the backend so the browser stays on same-origin requests; when deploying with `frontend/.env.pages`, leave `VITE_API_BASE` empty and make sure `pages/functions/_middleware.js` forwards `/admin/*`, `/open_api/*`, and related paths to `BACKEND` |
